...
class Hyperspreading:
    ...
    def findAdjNode_CP(inode, df_hyper_matrix):
        ...
        - array-like, 相邻的节点集合
        """
        edges_set = Hyperspreading.getHpe(inode, df_hyper_matrix)
        edge = Hyperspreading.chooseHpe(edges_set)
        adj_nodes = np.array(Hyperspreading.getNodesofHpe(edge, df_hyper_matrix))
        return adj_nodes
    ...
    def hyperSI(self, df_hyper_matrix, seeds):
        ...
        for t in range(0, iters):
            ...
            I_list.extend(infected_T) ######有大量冗余，注意注意注意
