...
class Hyperspreading:
    def getHpe(inode, matrix):   #获取给定节点所在的超边的集合gethyperedge
        """
        获取与给定节点相连的节点集合
        参数:
        - inode: int, 节点编号
    ...
    def chooseHpe(hpe_set):         #随机选择一条超边 choosehyperedge
        """
        从给定节点集合中选择一个节点
        参数:
        - hpe_set: set, 节点集合
        ...
        if len(hpe_set) > 0:
            ...
            return []
    def getNodesofHpe(hpe, matrix):   #获取指定超边所覆盖的节点
        """
        获取与给定节点相连的节点集合
        参数:
        - hpe: int, 节点编号
    ...
    def getNodesofHpeSet(hpe_set, matrix):   #可以用来找所有的邻居节点
        """
    ...
    def getTrueStateNode(self, adj_nodes, I_list, R_list):     #未恢复的节点是什么意思？未感染、已感染、已恢复
        """
        获取未感染和未恢复节点集合
        参数:
    ...
    def hyperSI(self, df_hyper_matrix, seeds):
        ...
        beta = 0.01
        iters = 25           #25跳
        I_total_list = [1]     #记录每跳感染后，已感染节点的总数
