...
class dataload:
    def get_scale(dataset, beta):
        # Algorithms = ['DPSO_HEDV','Climb_NB','Climb','DPSO', 'HEDV-greedy', 'HADP', 'HSDP', 'H-RIS', 'H-CI(I=1)', 'H-CI(I=2)', 'H-Degree', 'Degree', 'General-greedy']
        # Algorithms = ['DPSO_HEDV']
        Algorithms = ['DPSO-HEDV_NG40H1','DPSO_NG40H1', 'HEDV-greedy', 'HADP', 'HSDP', 'H-RIS', 'H-CI(I=1)', 'H-CI(I=2)', 'H-Degree', 'Degree','Adeff','Hyper-IMRank']
        ...
        for col, algo in enumerate(result.columns):
            for row, size in enumerate(result.index):
                ...
                # data_path = '../2-simulation_experiment/t_beta = 25_%s/%s/%s_%s.txt'%(beta, dataset, algo, size)
